...
from axiom_core import __version__
from axiom_core.config import load_settings

app = typer.Typer(
    name="axiom_core",
    help="axiom-core: Axiom 的 Python 后端",
...
def serve(
    ...
    uvicorn.run("axiom_core.api.app:app", host=host, port=port, reload=reload)
...
def chat(
    ...
    需要 API key (AXIOM_MODELS__LARGE__API_KEY 环境变量或 config.toml)。
    """
    ...
    if not settings.models:
        typer.secho(
            ...
            "示例:\n"
            "  export AXIOM_MODELS__LARGE__MODEL=deepseek-chat\n"
            "  export AXIOM_MODELS__LARGE__BASE_URL=https://api.deepseek.com/v1\n"
            "  export AXIOM_MODELS__LARGE__API_KEY=sk-...",
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def run(
    ...
    示例:
      axiom run "列出当前目录文件并统计数量"
      axiom run "帮我规划一个数据分析任务" --plan-mode
    """
    from pathlib import Path
    ...
    from axiom_core.llm.openai_compat import OpenAICompatClient
    from axiom_core.settings import AppSettings, SettingsStore

    settings = load_settings()
    app_cfg = SettingsStore(settings.data_dir).load() or AppSettings()
    if not settings.models:
        typer.secho("错误: 未配置模型 (设 AXIOM_MODELS__LARGE__* 环境变量)", fg=typer.colors.RED)
        raise typer.Exit(1)
...
def demo_survey(
    ...
    from axiom_core.scripts.ingest_survey import ingest_survey
    from axiom_core.settings import AppSettings, SettingsStore
    ...
    if not serve:
        typer.secho("\n跳过启动服务 (--no-serve)。手动起: axiom serve", fg=typer.colors.CYAN)
        return
    ...
    config = uvicorn.Config("axiom_core.api.app:app", host=host, port=port, log_level="warning")
    ...
    async def _serve_with_demo_session():
        ...
        try:
            ...
            from axiom_core.api.app import app as _app
            from axiom_core.api.sessions import SessionManager  # 复用 app 里的 manager
            from axiom_core.llm.openai_compat import OpenAICompatClient
