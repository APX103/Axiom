...
class VerificationConfig(BaseModel):
    ...
    reviewer_axiom_budget: int = 8  # 原 0039.js:229
...
class TraceConfig(BaseModel):
    """结构化 trace 配置。对照 axiom_core.observability.TraceRecorder。
...
class Settings(BaseSettings):
    ...
    环境变量前缀 AXIOM_ (如 AXIOM_DATA_DIR)。
    """
    ...
    db_path: Path | None = None  # None → data_dir/axiom_core.db
    ...
    def db_url(self) -> str:
        path = self.db_path or (self.data_dir / "axiom_core.db")
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    # 先拿一个默认 Settings 以确定 data_dir (可能被 AXIOM_DATA_DIR 覆盖)
    default_settings = Settings()

    # 2. APP 内持久化设置
    try:
        from axiom_core.settings import SettingsStore, app_settings_to_config_settings
