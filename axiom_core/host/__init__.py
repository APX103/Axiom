...
Python 全栈下,host 是进程内对象,直接调用 axiom_core 内部组件 (ArtifactStore/LLMClient 等),
...
class Host:
    """host 对象 (进程内)。对照原版 SDK (0221.js:2716)。
...
def make_host(
    *,
    ...
    本项目在主进程运行,不能污染 sys.modules (会让 axiom_core 包 import 失败)。
