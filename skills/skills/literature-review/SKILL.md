...
When the deliverable is a LaTeX survey/review paper (`.tex` + `.bib`), the \
citation format changes from the markdown-link convention above. The frontend \
renderer parses natbib `\cite{key}` commands and builds a numbered reference list \
from the `.bib` file — **markdown links and `[1]` numbering do not work in the \
renderer and will produce broken citations with an empty reference list.**

### Citation format (LaTeX mode)

- **Inline**: Use `\citep{key}` (parenthetical) or `\citet{key}` (textual). \
Example: `Early work on attention \citep{vaswani2017attention} established ...` \
renders as `... established ... [1]`.
- **Multiple**: `\citep{key1,key2}` — comma-separated, no spaces.
- **No markdown links, no `[1]` numbering, no `(Author Year)` inline text** in the \
`.tex` file. The renderer handles numbering automatically.

### BibTeX key convention

Derive keys deterministically from the retrieved paper metadata:
- Format: `firstauthor_lastname` + `year` + `first_content_word` \
(e.g. `wei2022chain`, `brown2020language`, `vaswani2017attention`)
- Lowercase, ASCII only. If the first author has a common name, add a second \
keyword for disambiguation.

### `.bib` entries

Write complete entries using metadata from `search_papers`/`fetch_paper`/`crossref_lookup`:
...
- **Never compose BibTeX from memory.** Use the author/year/title/DOI returned \
by the retrieval tools. If a field is missing from the tool output, omit it \
rather than guessing.
- Keep `\cite{key}` in the `.tex` and `@article{key,...}` in the `.bib` \
perfectly in sync — a key in one but not the other renders as `[?]`.
...
Save the `.tex` and `references.bib` as separate files in the workspace. The \
frontend's PaperView automatically detects `.tex` files and offers a preview; \
it matches each `.tex` with its同名 `.bib`.

## Style pass before saving

Before saving the artifact, run `style_pass(draft)` once on the full markdown. \
Fix the issues it lists in a single editing pass, then save; do not call it a \
second time and do not loop until it returns ok. It is a lint, not a gate, and \
a clean draft on the first pass is normal. It is shipped in this skill's \
`kernel.py` and auto-loaded; if `style_pass` is not in `dir()`, read `kernel.py` \
from this skill's directory and exec it. For LaTeX papers, the style pass \
operates on the `.tex` source — check for uncited `.bib` entries, `\cite` keys \
missing from the `.bib`, and markdown-style citations that should be `\citep`.
