---
name: self-awareness
description: the runtime's own session database schema and SDK surface for introspection via host.query(). Load this when you need to query your own conversation history, token usage, cost accounting, execution log, or artifact metadata beyond what host.frames()/host.artifacts() provide — e.g. "how many tokens has this session used", "what was my last tool call", "list every file I've written", "where are messages stored", "what tables can I query", "inspect frames.context_data", or any time you're about to PRAGMA-probe the runtime metadata DB to discover its schema.
license: Apache-2.0
---

# Self-awareness — the runtime's own database and SDK

`host.query(sql, params=[], limit=None, df=False)` runs read-only SQLite
against the runtime's own metadata DB. It is only available via the **`repl`
