...
detached wrapper survives an Operon daemon restart — on restart you'll see
...
warm containers from other Operon installs share the same app.
...
Operon-side kills (a queued `close()` is deferred ~15 min, then
...
under. Operon resolves every named object it touches (`App.lookup`,
